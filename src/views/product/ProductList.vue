...
const DeleteProduct = async (productId) => {
    try {
        loading.value = true;
        const respond = await showConfirm('Are you sure you want to delete this product? ');
    ...
    <div class="container py-4">
        ...
        <div v-else class="border rounded pb-3 px-2">
            <div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-md-center p-4">
                ...
                <router-link :to="ROUTE_NAMES.PRODUCT_CREATE" class="btn btn-success btn-sm gap-2 rounded-1 px-4 py-2">
                            ...
                            <tr v-for="product in products" :key="product.id">
                                <td class="ps-3">
                                    <div class="d-flex align-items-center">
                                        <img :src="product.image || `https://placehold.co/50x50`"
                                            class="rounded object-fit-cover me-2" style="width: 50px; height: 50px" />
                                        <div>
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        ...
                                        <span class="text-danger small" v-if="product.salePrice"> ${{ product.salePrice
                                            }} </span>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td class="pe-3 text-end">
                                    <button
                                        @click="router.push({ name: ROUTE_NAMES.PRODUCT_UPDATE, params: { id: product.id } })"
